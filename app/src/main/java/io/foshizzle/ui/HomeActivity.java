...
import io.foshizzle.data.DataManager;
...
import io.foshizzle.data.pocket.PocketUtils;
...
import io.foshizzle.ui.recyclerview.FilterTouchHelperCallback;
import io.foshizzle.ui.recyclerview.GridItemDividerDecoration;
import io.foshizzle.ui.recyclerview.InfiniteScrollListener;
import io.foshizzle.ui.transitions.MorphTransform;
import io.foshizzle.util.AnimUtils;
//import io.plaidapp.util.DrawableUtils;
import io.foshizzle.util.DrawableUtils;
import io.foshizzle.util.ViewUtils;

import static io.foshizzle.data.prefs.SourceManager.ACTIVE_SOURCE;
import static io.foshizzle.data.prefs.SourceManager.KEY_DEVIANTART;
import static io.foshizzle.data.prefs.SourceManager.KEY_DRIBBBLE;
import static io.foshizzle.data.prefs.SourceManager.KEY_SOURCES;
import static io.foshizzle.data.prefs.SourceManager.SOURCES_PREF;
...
public class HomeActivity extends Activity {
    ...
    private static final int RC_DEVIANTART_POPULAR = 4;
    private static final int RC_DEVIANTART_DAILY = 5;
    private static final int RC_DEVIANTART_GALLERY = 6;
    private static final int RC_DEVIANTART_FEATURED = 7;


    com.github.clans.fab.FloatingActionButton fabDribbble;
    com.github.clans.fab.FloatingActionButton fabDeviantart;
    FloatingActionMenu menuButtons;
...
//                fab.setLayoutParams(lpFab);
    ...
    public void setupMenuButtons() {
        ...
        menuButtons = (FloatingActionMenu) findViewById(R.id.menuButtons);
        fabDribbble = (FloatingActionButton) findViewById(R.id.fabDribbble);
        fabDeviantart = (FloatingActionButton) findViewById(R.id.fabDeviantart);

        fabDeviantart.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                SharedPreferences prefs = getApplicationContext().getSharedPreferences(SOURCES_PREF, Context.MODE_PRIVATE);

                if(!prefs.getString(ACTIVE_SOURCE, KEY_DEVIANTART).contains(KEY_DEVIANTART)) {
                    prefs.edit().clear().apply();

                    SharedPreferences.Editor editor = prefs.edit();

                    editor.putStringSet(KEY_SOURCES, null).apply();
                    editor.putString(ACTIVE_SOURCE, KEY_DEVIANTART).apply();

                    restartActivity(HomeActivity.this);
                }
        ...
        fabDribbble.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                SharedPreferences prefs = getApplicationContext().getSharedPreferences(SOURCES_PREF, Context.MODE_PRIVATE);

                if(!prefs.getString(ACTIVE_SOURCE, KEY_DRIBBBLE).contains(KEY_DRIBBBLE)) {
                    ...
                    editor.putStringSet(KEY_SOURCES, null).apply();
                    editor.putString(ACTIVE_SOURCE, KEY_DRIBBBLE).apply();
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.menu_dribbble_login:
                ...
                return true;
            case R.id.menu_deviantart_login:
...
//                    Source designerNewsSearch = null;
            ...
            case RC_DEVIANTART_DAILY:
                ...
                break;
            case RC_DEVIANTART_GALLERY:
                if (resultCode == RESULT_OK) {
                    filtersAdapter.enableFilterByKey(
                            SourceManager.SOURCE_DEVIANTART_GALLERY, this);
                }
                break;
            case RC_DEVIANTART_FEATURED:
                if (resultCode == RESULT_OK) {
                    filtersAdapter.enableFilterByKey(
                            SourceManager.SOURCE_DEVIANTART_FEATURED, this);
                }
                break;
    ...
    int getAuthSourceRequestCode(Source filter) {
        switch (filter.key) {
            ...
            case SourceManager.SOURCE_DEVIANTART_GALLERY:
                return  RC_DEVIANTART_GALLERY;
            case SourceManager.SOURCE_DEVIANTART_FEATURED:
                return RC_DEVIANTART_FEATURED;
        }
